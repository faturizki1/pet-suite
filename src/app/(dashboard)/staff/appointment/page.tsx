...
export default function StaffAppointmentPage() {
  ...
  return (
    <div>
        ...
        columns={[
          ...
          { key: "status", header: "Status", render: (a: Appointment) => <Badge status={a.status} /> },
          {
            ...
            render: (a: Appointment) => (
              <Button size="sm" variant="outline" onClick={() => { setSelected(a); setNewStatus(a.status); }}>
                Update
              </Button>
            ),
          },
      ...
      <Modal open={!!selected} onClose={() => setSelected(null)} title="Update Status Appointment">
        <div className="space-y-4">
          <p className="text-sm text-slate-500">
            {selected?.pet.nama} — {selected?.customer.namaLengkap}
          </p>
          <Select
            label="Status"
            options={[
